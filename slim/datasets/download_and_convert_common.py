...
def _get_dataset_filename(dataset_dir, split_name, shard_id, output_suffix=None):
    if output_suffix:
        output_filename = '%s_%s_%s_%05d-of-%05d.tfrecord' % (
            FLAGS.dataset_name, output_suffix, split_name, shard_id, _NUM_SHARDS)
    else:
...
def _get_dataset_filename_for_test(dataset_dir, shard_id):
    output_filename = '%s_test_%05d-of-%05d.tfrecord' % (FLAGS.dataset_name, shard_id, _NUM_SHARDS)
    return os.path.join(dataset_dir, output_filename)
